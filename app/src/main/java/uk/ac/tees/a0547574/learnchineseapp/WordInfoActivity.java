...
public class WordInfoActivity extends BaseActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String url = URLUtil.getWordurl(zi);  // 拼接网址
        initView();
//        初始化ListView显示的数据源
...
//        加载网络数据
        loadData(url);
//        调用判断是否已经收藏了的方法
        isExist = DBManage.isExistZiInCollwordtb(zi);
    ...
    public void onSuccess(String json) {
        ...
        // 插入数据库
        DBManage.insertWordToWordtb(resultBean);
        // 将数据显示在View控件上
...
//            联网失败，获取数据库当中字的信息
        WordBean.ResultBean bean = DBManage.queryWordFromWordtb(zi);
        if (bean!=null) {
            notifyView(bean);
        }
    }
...
//            原来数据收藏，后来不想收藏了，需要删除
            DBManage.deleteZiToCollwordtb(zi);
        }
        if (!isExist&&isCollect) {
//            原来不存在，后来需要收藏，要插入数据
            DBManage.insertZiToCollwordtb(zi);
        }
    }
}
