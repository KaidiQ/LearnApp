...
import uk.ac.tees.a0547574.learnchineseapp.Util.MyGridView;
import uk.ac.tees.a0547574.learnchineseapp.Util.URLUtil;

import java.util.ArrayList;
import java.util.List;

public class IdiomInfoActivity extends BaseActivity {
...
//        获取上一个页面传递的数据
        ...
        String url = URLUtil.getChengyuurl(chengyu);
        loadData(url);
        isExist = DBManage.isExistCyuInCollcyutb(chengyu);
        isCollect = isExist;
...
//        因数据源当中不包括成语本身，但是后期要插入数据库，所以需要保存这个成语
            ...
            // 插入到数据库当中
            DBManage.insertCyToCyutb(cyBean);
            // 显示数据
...
//        获取数据库当中缓存的数据
        ChengyuBean.ResultBean bean = DBManage.queryCyFromCyutb(chengyu);
        if (bean!=null) {
            showDataToView(bean);
        }
    }
...
//        判断是否有同义词
    ...
    protected void onDestroy() {
        ...
        if (isExist&&!isCollect) {
            DBManage.deleteCyuToCollcyutb(chengyu);
        }
        if (!isExist&&isCollect) {
            DBManage.insertCyuToCollcyutb(chengyu);
        }
    }
}
