...
import uk.ac.tees.a0547574.learnchineseapp.Bean.PinBuBean;
...
import uk.ac.tees.a0547574.learnchineseapp.Util.AssetsUtil;
import uk.ac.tees.a0547574.learnchineseapp.Util.CommonUtils;
import uk.ac.tees.a0547574.learnchineseapp.Util.URLUtil;
import uk.ac.tees.a0547574.learnchineseapp.adapter.SearchLeftAdapter;
import uk.ac.tees.a0547574.learnchineseapp.adapter.SearchRightAdapter;
...
public class BaseSearchActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        exLv = findViewById(R.id.search_elv);
        pullGv = findViewById(R.id.search_gv);
        titleTv = findViewById(R.id.search_tv);
        // 初始化GriView的数据源内容
        initGridDatas();
    }
...
//        将加载到的网络数据写入到数据库当中
    ...
    public void writeDBByThread(final List<PinBuWordBean.ResultBean.ListBean> list) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                DBManage.insertListToPywordtb(list);
            }
        }).start();
    }
    /**
...
//                获取被点击位置的内容
    ...
    public void initData(String assetsName,int type) {
        ...
        String json = AssetsUtil.getAssetsContent(this, assetsName);
